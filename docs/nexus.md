...
这份文档说明 `apps/nexus` 后续怎么组织。

## 后续保留内容

`@xdd-zone/nexus` 是 Hono API 服务，运行在 Node.js 上。
...
`AppType` 要保留。Console 后续可以用它拿到 Nexus 的路由类型。
...
`<module>.types.ts` 只放 Nexus 内部使用的模块类型。
...
```text
apps/nexus/src/infra
```

数据库相关代码放在：

```text
apps/nexus/src/infra/db
```
...
- `env.ts`
  读取和校验 Node 环境变量。当前 Nexus 使用 `APP_ENV` 和 `PORT`。新增环境变量时先改这里。
- `hono-env.ts`
  定义 Hono 的 `Bindings` 和 `Variables` 类型。所有 `new Hono()` 都使用 `new Hono<HonoEnv>()`。
- `meta.ts`
  生成接口响应里的 `meta`。当前包含 `requestId` 和 `timestamp`。
- `response.ts`
  放 Nexus 内部响应辅助函数。如果 route 里反复调用 `buildSuccess()` 或 `buildFailure()`，可以在这里封一层短函数。
- `validator.ts`
...
2. 在 `apps/nexus/src/modules/<module>/<module>.route.ts` 添加 Hono route。
3. 需要业务判断时，在同目录添加或修改 `<module>.service.ts`。
4. 需要数据库读写时，在同目录添加或修改 `<module>.repository.ts`。
5. 在 `apps/nexus/src/routes/index.ts` 挂载模块路由。
6. 在 `apps/nexus/src/test/<module>.test.ts` 添加接口测试。
...
```text
apps/nexus/src/test/system.test.ts
apps/nexus/src/test/auth.test.ts
apps/nexus/src/test/user.test.ts
```
