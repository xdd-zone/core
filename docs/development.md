...
code-server Web IDE 里如果要通过个人 dev 域名访问本机服务，也用同一套 `.env.development` 和 `pnpm dev`。dev 域名、frp 和 Caddy 的配置维护在个人目录 `~/Projects/code-server-frp-maintenance`，不在这个仓库里。
...
新增接口的完整步骤和分层规则看 [apps/momo.md](./apps/momo.md) 的「新增接口」和「模块写法」，Fifa 侧的接口函数和 query hook 写法看 [apps/fifa.md](./apps/fifa.md) 的「Momo 请求」。
