...
- 触发预览、发布和重新刷新缓存。
- 展示保存、发布、刷新失败等操作结果。
- 检查 Momo 外部依赖状态，查看并重试 outbox 任务。
...
`system` 提供进程状态和依赖检查。`GET /health` 只判断 Momo 进程是否能响应，`GET /rpc/system/readiness` 检查 PostgreSQL、缓存、搜索和文件存储。

`events` 管理发布和归档产生的 `event_outbox`。Fifa 可以读取任务列表、查看失败原因并重试单条任务。通用 Pino 日志仍写到标准输出，由部署环境保存；Momo 不读取本地日志文件，也不把每条请求日志写入业务数据库。
