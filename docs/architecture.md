...
这里放文章草稿、已发布文章、预览 token、分类、标签、MDX 组件清单，以及和文稿直接绑定的发布流程。
...
适合放 `siteKey = "bobo"`、导航项、首页模块开关和排序、SEO 默认值，以及 RSS、sitemap 和搜索需要的站点配置。
...
发布成功后会有派生任务：刷新 Bobo 缓存、更新搜索索引、刷新 sitemap 和 RSS，以及让相关预览 token 失效。
...
存储能力保持简单：保存、读取、删除文件，再加上生成公开地址或读取地址，就够了。
