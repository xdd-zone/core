...
`BETTER_AUTH_URL` 会参与拼接 Better Auth 回调地址。这里填 `http://localhost:7788` 时，Google 后台也必须填 `http://localhost:7788/api/auth/callback/google`。code-server Web IDE 里使用个人 dev 域名时，按实际 Momo 地址添加 `/api/auth/callback/google`。
