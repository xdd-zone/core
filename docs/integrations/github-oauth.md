...
Momo 支持通过 `better-auth` 启用 GitHub 登录。GitHub OAuth 凭证留空时，只保留 email/password 登录。
...
`GITHUB_CLIENT_ID` 和 `GITHUB_CLIENT_SECRET` 必须同时填写或同时留空。只填一个时 Momo 会拒绝启动。本地启用 GitHub 登录后，还要保证 `CORS_ORIGINS` 里包含前端地址。
...
`BETTER_AUTH_URL` 会参与拼接 Better Auth 回调地址。这里填 `http://localhost:7788` 时，GitHub 后台也必须填 `http://localhost:7788/api/auth/callback/github`。code-server Web IDE 里使用个人 dev 域名时，按实际 Momo 地址添加 `/api/auth/callback/github`。
