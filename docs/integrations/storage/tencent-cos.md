...
- `packages/nexus/src/core/config/schema.ts`
  校验 `storage` 和 `COS_*` 配置。
- `packages/nexus/src/core/config/load-config.ts`
  从环境变量读取 `STORAGE_PROVIDER`、`COS_SECRET_ID`、`COS_SECRET_KEY`、`COS_BUCKET`、`COS_REGION`。
- `packages/nexus/src/infra/storage/media-storage.ts`
  按 `storage.provider` 选择本地存储或 COS 存储。
- `packages/nexus/src/infra/storage/local-media-storage.ts`
  本地文件存储，文件写到 `packages/nexus/storage/media`。
- `packages/nexus/src/infra/storage/cos-media-storage.ts`
  COS 文件存储，调用 `cos-nodejs-sdk-v5` 的 `putObject`、`getObject`、`deleteObject`。
- `packages/nexus/src/modules/media/service.ts`
