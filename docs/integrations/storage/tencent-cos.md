...
- 当前代码里的 `storagePath` 就是 COS 对象 `Key`。它由 `COS_KEY_PREFIX` 和生成后的文件名拼出来，例如 `media/<uuid>.png`。
...
脚本验证 COS 时会真实调用 `putObject` 和 `deleteObject`。`openFile()` 只检查返回的 `302` 和 `location`，不会把带签名参数的完整地址打印到终端。
