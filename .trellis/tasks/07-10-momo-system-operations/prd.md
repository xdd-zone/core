...
给 Fifa owner 提供一个系统运行页面，用来检查 Momo 依赖状态、查看发布后任务，并重试单条失败任务。通用 Pino 日志仍然写到标准输出，不在本次写入 PostgreSQL 或读取本地日志文件。
...
- 文档说明通用 Pino 日志仍由部署环境保存，本次不提供原始日志读取接口。
