...
import { createChildLogger } from '#momo/infra/logger'
...
if (isEntry) {
  ...
  const serverLogger = createChildLogger(runtime.logger, 'server')
  ...
  const shutdown = () => {
    serverLogger.info({ event: 'server.shutting_down' }, 'Momo Hono 服务正在关闭...')
    server.close(async (err) => {
      if (err) {
        ...
        serverLogger.info({ event: 'server.http_closed' }, 'HTTP 服务已关闭')
      }

      try {
        await runtime.cache.close()
        serverLogger.info({ event: 'server.cache_closed' }, '缓存连接已清理')
      } catch (cacheErr) {
        serverLogger.error({ errorMessage: String(cacheErr), event: 'server.cache_close_failed' }, '清理缓存连接异常')
      }

      try {
        await closeDb()
        serverLogger.info({ event: 'server.db_closed' }, '数据库连接已清理')
      } catch (dbErr) {
        serverLogger.error({ errorMessage: String(dbErr), event: 'server.db_close_failed' }, '清理数据库连接异常')
      }

      serverLogger.info({ event: 'server.shutdown_complete' }, 'Momo Hono 服务已完全关闭')
