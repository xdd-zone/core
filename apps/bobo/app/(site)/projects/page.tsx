...
export default async function ProjectsPage() {
  const projects = await getPublicProjectsOrFallback()
        ...
        <div className="max-w-300 mx-auto px-6">
          {projects.length > 0 ? (
              ...
              {projects.map((project) => (
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    {project.description ? (
                      <p className="mt-4 text-muted-foreground text-[0.92rem] leading-[1.75]">{project.description}</p>
                    ) : null}
                  </div>
                </Link>
              ))}
            ...
            <div className="border-t border-border py-9">
              <p className="text-muted-foreground">当前还没有公开项目。</p>
            </div>
          )}
        </div>
      </section>
