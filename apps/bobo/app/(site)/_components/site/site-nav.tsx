...
export function SiteNav({ activeHref, categories = [] }: SiteNavProps) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center py-6 px-4">
      <div className="site-nav-pill scrolled relative inline-flex items-center gap-0.5 rounded-full backdrop-blur-md border border-white/10 bg-surface p-2 transition-[box-shadow,transform] duration-360 ease-[cubic-bezier(0.23,1,0.32,1)] [&.scrolled]:shadow-[0_8px_24px_rgba(0,0,0,0.4)] [&.scrolled]:-translate-y-0.5">
        <Link
