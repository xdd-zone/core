...
export async function generateMetadata({ params, searchParams }: GenericPreviewPageProps): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: preview.targetType === 'project' ? preview.project.draft.title : preview.post.revision.title,
      robots: {
        follow: false,
        index: false,
      },
...
export default async function GenericPreviewPage({ params, searchParams }: GenericPreviewPageProps) {
  ...
  return <PostPreview preview={preview.post} />
}
...
function PostPreview({
  ...
  preview: { revision: { excerpt: string | null; revisionNo: number; source: string; title: string } }
}) {
  return (
...
function ProjectPreview({
  ...
  project: { draft: { description: string | null; links: { href: string; label: string }[]; title: string } }
}) {
  return (
    <main className="min-h-screen px-5 py-6 md:px-8 md:py-8">
      <div className="mx-auto w-full max-w-3xl">
        ...
        <header className="border-b border-border/70 py-10 md:py-14">
          <p className="text-xs font-medium tracking-[0.24em] text-fg-muted uppercase">preview</p>
          <h1 className="mt-4 text-4xl font-semibold tracking-tight text-fg md:text-5xl">{project.draft.title}</h1>
