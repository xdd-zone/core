...
const LEGACY_HOME_TAB_LABELS = new Set(['menu.dashboard', DEFAULT_HOME_TAB.label])
const LEGACY_HOME_TAB_PATHS = new Set(['/dashboard', DEFAULT_HOME_TAB.path])

function isHomeTab(tab: Tab): boolean {
  return LEGACY_HOME_TAB_IDS.has(tab.id) || LEGACY_HOME_TAB_LABELS.has(tab.label) || LEGACY_HOME_TAB_PATHS.has(tab.path)
}
  ...
  persist(
    (set, get) => ({
      ...
      addOrActivateTab: (tab: Tab) => {
        ...
        const existingTab = normalizedTabs.find((t) => t.path === normalizedTab.path)
