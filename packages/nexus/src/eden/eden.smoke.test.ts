...
const EDEN_SMOKE_PATHS: EdenSmokePathGroup[] = [
  {
    label: 'auth',
    paths: ['/api/auth/get-session', '/api/auth/me', '/api/auth/sign-in/email', '/api/auth/sign-up/email', '/api/auth/sign-out'],
  },
  {
    label: 'user',
    paths: ['/api/user'],
  ...
  {
    label: 'public-site',
    paths: ['/api/public-site/posts', '/api/public-site/posts/:slug'],
  },
  {
    label: 'health',
    paths: ['/api/health'],
...
describe('eden smoke', () => {
  ...
  describe('auth', () => {
    it('smoke 路径白名单至少覆盖当前认证入口', () => {
      expect(EDEN_SMOKE_PATHS).toContainEqual(
        expect.objectContaining({
          label: 'auth',
          paths: expect.arrayContaining(['/api/auth/get-session', '/api/auth/me', '/api/auth/sign-in/email']),
        }),
      )
    })
    ...
    it('邮箱注册后应带登录态和完整关键字段', async () => {
      ...
      expect(meResult.data?.user?.email).toBe(credentials.email)
      expect(meResult.data?.user?.name).toBe(credentials.name)
      expect(meResult.data?.session?.userId).toBe(userId)
      expect(typeof meResult.data?.session?.id).toBe('string')
      expectDateTime(meResult.data?.session?.expiresAt)
    })
    ...
    it('邮箱登录后应可登出且返回 204 空 body', async () => {
      ...
      expect(signInResult.data?.user.name).toBe(credentials.name)
  ...
  describe('user', () => {
    it('后台用户列表应守住匿名 401、登录无权限 403 和授权后 200 契约', async () => {
      ...
      expect(typeof allowedResult.data?.totalPages).toBe('number')
      expect(
        allowedResult.data?.items.some((item) => item.id === privilegedUserId && item.email === privilegedCredentials.email),
      ).toBe(true)
    })
  })

  describe('public-site', () => {
    it('smoke 路径白名单至少覆盖当前公开站入口', () => {
      expect(EDEN_SMOKE_PATHS).toContainEqual(
        expect.objectContaining({
          label: 'public-site',
          paths: expect.arrayContaining(['/api/public-site/posts', '/api/public-site/posts/:slug']),
        }),
      )
    })
    ...
    it('公开站点应只暴露公开文章契约字段', async () => {
      ...
      expectDateTime(item?.createdAt)
      expectDateTime(item?.updatedAt)
      ...
      expectDateTime(detailResult.data?.publishedAt)
    })
  })
})
