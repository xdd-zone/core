...
describe('openapi smoke', () => {
  it('应导出当前关键 API 路径', async () => {
    ...
    expect(paths['/api/public-site/config']).toBeTruthy()
    expect(paths['/api/public-site/categories']).toBeTruthy()
    expect(paths['/api/public-site/categories/{slug}/posts']).toBeTruthy()
    expect(paths['/api/public-site/posts']).toBeTruthy()
    expect(paths['/api/public-site/posts/{slug}']).toBeTruthy()
