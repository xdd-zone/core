...
describe('openapi smoke', () => {
  it('应导出当前关键 API 路径', async () => {
    ...
    expect(paths['/api/page/']).toBeTruthy()
    expect(paths['/api/page/{id}']).toBeTruthy()
    expect(paths['/api/page/{id}/publish']).toBeTruthy()
    expect(paths['/api/page/{id}/unpublish']).toBeTruthy()
